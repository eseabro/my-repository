...
% 
% <<D:\Documents-Emma\Year2\MAT292\Matlab labs\html\ex1_seabro13.png>>
% 
%
...
% your answer in (b).
%   The solution to this ODE is y=e^(-t/2)(c1*cos(2t)+c2*sin(2t)). As t->
%   infinity, the e^(-t/2) term approaches 0 which means the value of the
%   entire solution will too. Because this solution is some combination of
%   sin(2t) and cos(2t) (depending on ICs), the solution will oscillate.
...
% 
% <<D:\Documents-Emma\Year2\MAT292\Matlab labs\html\ex2_seabro13.png>> 
%
% (b) 100% of the solutions grow (c) The solution to
% this ODE is y= c1*e^(((-sqrt(3)+2)t/2)) +c2*e^(-(sqrt(3)+2)t/2) which
% explains the behaviour of the solutions because the value of y will
% continue to grow with t because of the e^t and e^-t terms. Also, the
% constants c1 and c2 are determined by the ICs. These constants determine
% whether the solution will grow towards -INF or +INF.
...
% Repeat (a), (b), (c) from Exercise 1 with this DE.
% 
% <<D:\Documents-Emma\Year2\MAT292\Matlab labs\html\ex3_seabro13.png>> 
%
% (b) 100% of the solutions decay (c) The solution to
% this ODE is y(x) = c1*e^(-1/2 (sqrt(2) + sqrt(3))t) + c2*e^(1/2 (sqrt(2)
% - sqrt(3))t) which explains the behaviour of the solutions because both
% of the parts of the solution are of the form e^(-t) which tends to 0 as
% t-> inf. Also, the constants c1 and c2 are determined by the ICs.
...
%  The general solution to this ODE is y(x) =
% c1*e^(-x)*sin(2*x)+ c2*e^(-x)*cos(2*x) + c3*cos(x)+c4*sin(x) 
% 
%
...
%   The solutions to this ODE will oscillate, but they will neither grow
%   nor decay. The oscillation is caused by the c3*cos(x)+c4*sin(x) terms
%   and the first two terms approach 0 around t=10 and don't have much of
%   an effect on the overall solution as it approaches inf.
...
%
%
% <<D:\Documents-Emma\Year2\MAT292\Matlab labs\html\ex7_seabro13.png>>
...
% My approximation fit the runge-kutta approximation so well that I have to
% zoom in a lot just to see the difference between the two. These two are a
