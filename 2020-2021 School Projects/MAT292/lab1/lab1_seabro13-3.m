...
% work as:
%
% Error for negative index:"Array indices must be positive integers or
% logical values."
%
...
disp("For loop time is:");
disp(elapsed);
clearvars y;
start = cputime;
y = atan(x);
elapsed = cputime - start;
disp("Vectorization time is:");
disp(elapsed);
...
% dy/dt + (2*t)*y = 1/exp(t^2)
% u(t) = exp(t^2)
% soln = y(t) = (1/exp(t^2))(t+C)
% y(1) = -1, y(1) = 0,
% and y(1) = 1
% indep var is 1-5
t = linspace(1,5,100);
sol_one = (1./exp(t.^2)).*(t-exp(1)-1);
sol_two = (1./exp(t.^2)).*(t-1);
sol_three = (1./exp(t.^2)).*(t+exp(1)-1);
plot(t,sol_one, t, sol_two, t, sol_three);
xlabel('t');
ylabel('y(t)');
title('Plot of solutions of y(t)');
legend('y(1)=-1', 'y(1)=0','y(1)=1', 'Location','SouthEast');
...
%
clear;
f = @(x,y)  y + y.^3 - cos(x) + x.^2 - x.^4;    %Inline function definition
disp(f(0, 0));                                  % Showing the solution to f(0,0)
disp(f(pi/2, 1));                               % Showing the solution to f(pi/2, 1)
disp(f(-1, -1));                                % Showing the solution to f(-1, -1)
...
clear;
% f = @(x,y) (-cos(x) + 3*x^2)* y;
% implicit solution: y_x = @(x) (exp(x^3-sin(x))+C);
% 
f = @(x) ((x^3-sin(x))+0);
g = @(x,y) (x^3 -sin(x) -log(abs(y)));
...
for j = 1:length(xvals)
   ...
   yvals(j) = fzero(@(y) g(xvals(j),y), 0.9);
...
%% Ans:
% The solution is -1/[(x^2/2)+C].
% As x-> -INF, y-> 0; As x->INF, y->0; As x->0(from left) y-> 1/C. As
% x->0(from right) y-> 1/C; where C is a constant. There are also two
% vertical asymptotes also at +/-[C/sqrt(2)].
% Because the only instance of x in the solution is squared in the
% solution, y(x) = y(-x) since (x)^2 = (-x)^2. 
%% Exercise 12
%
...
%% Ans:
% when y=2; y->2 as x->inf; y->2 as x->-inf
% when (-1<y_0<2); y->1 as x->inf; [-1<y_0<2)y->-1 as x->-inf
% when (-2.3<y_0<-1]; y->-1 as x->inf; (-2.3<y_0<-1)y->-inf as x->0 
%when (-3<y_0<-2.3]; y->1 as x->inf; y->-inf as x->0 
%
% The solutions cannot cross because of the Existence and Uniqueness
% Theorem. Stated by the theorem, there should be a unique solution to the
% ODE for every value of t and y0. Therefore, if two solutions were to
% cross, the intersection could be used as y0 which would contradict the
% theorem as there would not be a unique function for that value of y0.
