...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            % Use occupancy grid to estimate laser reading

            if isnan(y_laser(i, j))
                continue;
            end

            robot_pos_pix_x = round((x_particle(n)-ogxmin)/ogres);
            robot_pos_pix_y = round((y_particle(n)-ogymin)/ogres);
            
            ang_beam = theta_particle(n) + angles(j);

            obs_x = (r_max_laser*cos(ang_beam))/ogres; % no. cells in x dir
            obs_y = (r_max_laser*sin(ang_beam))/ogres; % no. cells in y dir

            laser_x = robot_pos_pix_x+obs_x;
            laser_y = robot_pos_pix_y+obs_y;

            d_x = obs_x/round(r_max_laser/ogres);
            d_y = obs_y/round(r_max_laser/ogres);

            curr_pt_x = robot_pos_pix_x + d_x;
            curr_pt_y = robot_pos_pix_y + d_y;

            found = 0;

            for k=1:round(r_max_laser/ogres)
                curr_pt_x = curr_pt_x + d_x;
                curr_pt_y = curr_pt_y + d_y;

                if (curr_pt_y >= size(ogp,1)) || (curr_pt_x >= size(ogp,2))
                    break
                end

                obstacle = ogp(round(curr_pt_y), round(curr_pt_x));

                if (obstacle >= 0.5) && (found==0)
                    dist_x = (curr_pt_x - robot_pos_pix_x)*ogres;
                    dist_y = (curr_pt_y - robot_pos_pix_y)*ogres;
                    laser_pred = sqrt(dist_x^2 + dist_y^2);
                    found = 1;
                    break;
                end

            end
            w_particle(n) = w_particle(n)*w_gain*normpdf(y_laser(i,j), laser_pred, sqrt(laser_var));
