...
class PathFollower():
    ...
    def update_pos(self):
        point = Odometry.pose.pose.position
        quart = Odometry.pose.pose.orientation
        theta = self.get_yaw_from_quaternion(quart)
    ...
    def follow_path(self):
        while not rospy.is_shutdown():
            ...
            for t in range(1, len(self.path_tuples) + 1):


                next_pt = self.path_tuples[t]
                
                d_x0 = curr_pt[0] - next_pt[0]
                d_y0 = curr_pt[1] - next_pt[1]
                theta_diff = np.arctan2(d_y0, d_x0)

                while abs(theta_diff) >= 0:
                    self.cmd_pub.publish(utils.unicyle_vel_to_twist([0, 1]))
                    rate.sleep(100)


                    curr_pt = self.update_pos()
                    d_x0 = next_pt[0] - curr_pt[0]
                    d_y0 = next_pt[1] - curr_pt[1]
                    theta_diff = np.arctan2(d_y0, d_x0)

                tw.angular.z = 0
                rospy.loginfo(tw.angular)
                pub.publish(tw)
