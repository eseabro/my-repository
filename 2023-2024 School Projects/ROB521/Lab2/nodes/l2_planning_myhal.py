...
class PathPlanner:
    ...
    def __init__(self, map_filename, map_setings_filename, goal_point, stopping_dist):
        ...
        self.vel_max = 0.1 #0.5 #m/s (Feel free to change!)
        ...
        self.timestep = 1.0 #s
        self.num_substeps = int(self.vel_max*self.timestep/self.map_settings_dict["resolution"])*10

        #Planning storage
        self.nodes = [Node([-0.1,0,0], -1, 0)]
        ...
        self.window = pygame_utils.PygameWindow(
            "Path Planner", (350, 100), self.occupancy_map.shape, self.map_settings_dict, self.goal_point, self.stopping_dist)
        return
    ...
    def sample_map_space(self):
        ...
        x_vals_m = np.linspace(-0.1, 2.2, 500)
        y_vals_m = np.linspace(0, 0.6, 100)

        x, y = 0, 0
        while self.check_if_duplicate([x,y]):
            if self.map_settings_dict["image"] == "myhal.pgm":
                x = random.choice(x_vals_m)
                y = random.choice(y_vals_m)
            elif self.sample_radius < 500:
    ...
    def point_to_cell_myhal(self, point):
        #Convert a series of [x,y] points in the map to the indices for the corresponding cell in the occupancy map
        #point is a 2 by N matrix of points of interest
        res = self.map_settings_dict["resolution"]
        xres = (2.4+0.1)/self.map_shape[0]
        yres = 0.6/self.map_shape[1]
        x = point[0] - self.map_settings_dict["origin"][0]
        y = point[1]
        h = self.map_shape[1] * res

        y_new = self.map_shape[1] - int(y/yres)
        x_new = int((x)/xres)
    ...
    def points_to_robot_circle(self, points):
        ...
        out_rr = []
        out_cc = []
        to_x = np.linspace(-0.1, 2, 100)
        to_y = np.zeros(100)
        points = np.transpose([to_x, to_y])

        for pt in points:
            cc = self.point_to_cell_myhal(pt)
            if (cc[0]>self.map_shape[0]) or (cc[1] > self.map_shape[1]) or (cc[0]<0) or (cc[1]<0):
                print("max")
                print(self.map_shape)
                return [], []
            d_rr, d_cc = disk(cc, self.robot_radius/self.map_settings_dict["resolution"], shape=np.shape(self.occupancy_map))
            if (d_rr.size == 0) or (d_cc.size == 0):
                print(d_rr)
                print(cc)
                print(pt)
                input("d_rr_size failure ")
                raise
            out_rr.append(d_rr)
            out_cc.append(d_cc)
    ...
    def rrt_planning(self):
        for i in range(50000): #Most likely need more iterations than this to complete the map!
            ...
            try:
                path_pts = []
                for rr, cc in zip(out_rr, out_cc):
                    path_pts.append(self.occupancy_map[rr, cc])
    ...
    def recover_path(self, algo_type, node_id = -1):
        ...
        path.append([0,0,0])
        path.reverse()
        pygame.image.save(self.window.screen, f"C:/Users/Emma/Documents/SchoolWork/Fourth Year/ROB521/Lab2/nodes/algo_result_{algo_type}_m.jpeg")
...
def main():
    ...
    if algo_type == "RRT":
        ...
        np.save("shortest_path_rrt_myhal.npy", node_path_metric_rrt)
        
    elif algo_type == "RRT_STAR":
        nodes = path_planner.rrt_star_planning()
        node_path_metric_rrt_star = np.array(path_planner.recover_path(algo_type))
        np.save("shortest_path_rrt_star_myhal.npy", node_path_metric_rrt_star)
