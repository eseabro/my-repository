...
class PathFollower():
    ...
    def follow_path(self):
        ...
        while not rospy.is_shutdown():
            ...
            curr_loc = self.pose_in_map_np
            print(curr_loc)
            print(next_pt)

            dx = next_pt[0] - curr_loc[0]
            dy = next_pt[1] - curr_loc[1]
            d_theta = abs(np.arctan2(dy, dx))
            print(d_theta)
            ...
            if self.eucl_distance(next_pt, curr_loc) <= 0.5:
                next_pt = self.path_tuples[i]
                i += 1
                control = [0, 0]
                print("new pt")
