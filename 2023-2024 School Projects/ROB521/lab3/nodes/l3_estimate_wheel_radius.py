...
class wheelRadiusEstimator():
    ...
    def startStopCallback(self, msg):
        ...
        if self.isMoving is False and np.absolute(input_velocity_mag) > 0:
            ...
            self.l_init = self.left_encoder_prev
            self.r_init = self.right_encoder_prev
            ...
            avg_ticks = (self.left_encoder_prev-self.l_init + self.right_encoder_prev-self.r_init) / 2
            ticks_rad = TICKS_PER_ROTATION/(2*np.pi)
            dist = avg_ticks/ticks_rad
            radius = DRIVEN_DISTANCE/dist
