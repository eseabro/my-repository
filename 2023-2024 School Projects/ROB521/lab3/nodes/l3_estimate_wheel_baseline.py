...
class wheelBaselineEstimator():
    def __init__(self):
        ...
        print('Ready to start wheel distance calibration!')
        return
    ...
    def startStopCallback(self, msg):
        if self.isMoving is False and np.absolute(msg.angular.z) > 0:
            self.rinit = self.right_encoder_prev
            self.linit = self.left_encoder_prev
            ...
            ticks_rad = TICKS_PER_ROTATION/(2*np.pi)

            rads_rotated_l = (self.left_encoder_prev-self.linit)/ticks_rad
            rads_rotated_r = (self.right_encoder_prev-self.rinit)/ticks_rad
            rads_rotated = 0.5*(rads_rotated_l-rads_rotated_r)


            distance_radians = WHEEL_RADIUS*rads_rotated
            

            separation = (distance_radians)/(NUM_ROTATIONS*2*np.pi)
