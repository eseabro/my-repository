...
class WheelOdom:
    ...
    def sensor_state_cb(self, sensor_state_msg):
        ...
        if self.last_enc_l is None:
            ...
            del_left_angle = (le - self.last_enc_l) * RAD_PER_TICK
            del_right_angle = (re - self.last_enc_r) * RAD_PER_TICK

            del_theta = (WHEEL_RADIUS / (2 * BASELINE)) * (del_right_angle - del_left_angle)

            theta = euler_from_ros_quat(self.pose.orientation)[2] + del_theta

            dv = (WHEEL_RADIUS / 2) * (del_right_angle + del_left_angle)
            dx = np.cos(theta) * dv
            dy = np.sin(theta) * dv
            ...
            self.twist.linear.x = dv / dt
            self.twist.linear.y = 0
            self.twist.angular.z = del_theta / dt
