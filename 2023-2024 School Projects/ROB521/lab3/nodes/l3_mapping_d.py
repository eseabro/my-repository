...
class OccupancyGripMap:
    ...
    def scan_cb(self, scan_msg):
        ...
        x = odom_map[0] # - self.map_settings_dict["origin"][0]
        y = odom_map[1] # - self.map_settings_dict["origin"][1]

        r_x = int(MAP_DIM[0]/CELL_SIZE - y/CELL_SIZE)
        r_y = int(x/CELL_SIZE)
    ...
    def ray_trace_update(self, map, log_odds, x_start, y_start, angle, range_mes):
        ...
        pix_x = x_start + int(x_dist/CELL_SIZE)
        pix_y = y_start + int(y_dist/CELL_SIZE)

        if pix_x >= np.shape(map)[0]:
            pix_x = int(np.shape(map)[0] - 1)
        if pix_y >= np.shape(map)[1]:
            pix_y = int(np.shape(map)[1] - 1)


        rr, cc = ray_trace(x_start, y_start, pix_x, pix_y)
        log_odds[rr, cc] -= BETA
        log_odds[pix_x, pix_y] += (2*ALPHA + BETA)
        map[rr,cc] = self.log_odds_to_probability(log_odds[rr,cc])
        map[pix_x, pix_y] = self.log_odds_to_probability(log_odds[pix_x, pix_y])
