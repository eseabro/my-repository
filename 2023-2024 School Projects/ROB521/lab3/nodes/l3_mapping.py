...
class OccupancyGripMap:
    ...
    def scan_cb(self, scan_msg):
        ...
        # inc = 0
        # for range_mes in scan_msg.ranges:
        #     angle = scan_msg.angle_increment*inc + odom_map[2] + np.pi/2
        #     self.np_map, self.log_odds = self.ray_trace_update(self.np_map, self.log_odds, 
        #                                                     x_start, y_start, angle, 
        #                                                     range_mes)
        #     inc += 1
        #     if round((scan_msg.angle_max - scan_msg.angle_increment*inc), 5) == 0:
        #         inc = 0

        for i, range_mes in enumerate(scan_msg.ranges):
            if (range_mes > scan_msg.range_min) & (range_mes < scan_msg.range_max):
                angle = scan_msg.angle_increment*i + odom_map[2]
                self.np_map, self.log_odds = self.ray_trace_update(self.np_map, self.log_odds, 
                                                                x_start, y_start, angle, 
                                                                range_mes)
    ...
    def ray_trace_update(self, map, log_odds, x_start, y_start, angle, range_mes):
        ...
        # x_dist = range_mes * np.cos(angle)
        # y_dist = range_mes * np.sin(angle)

        # x_end = x_start + x_dist
        # y_end = y_start + y_dist

        # x_start_map = int(y_start/CELL_SIZE)
        # y_start_map = int(x_start/CELL_SIZE)

        # x_end_map = int(y_end/CELL_SIZE)
        # y_end_map = int(x_end/CELL_SIZE)

        # if x_end_map >= map.shape[0]:
        #     x_end_map = int(1)
        # if y_end_map >= map.shape[1]:
        #     y_end_map = int(map.shape[1] - 1)

        # rr, cc = ray_trace(x_start_map, y_start_map, x_end_map, y_end_map)

        # valid_indices = (rr >= 0) & (rr < map.shape[0]) & (cc >= 0) & (cc < map.shape[1])
        # valid_rr, valid_cc = rr[valid_indices], cc[valid_indices]

        # log_odds[valid_rr, valid_cc] -= BETA
        # log_odds[x_end_map, y_end_map] += (2*ALPHA + BETA)
        # map[valid_rr, valid_cc] = self.log_odds_to_probability(log_odds[valid_rr, valid_cc])
        # map[x_end_map, y_end_map] = self.log_odds_to_probability(log_odds[x_end_map, y_end_map])

        # Calculate start and end of obstacle
        x_obs_start_dist = range_mes * np.cos(angle)
        y_obs_start_dist = range_mes * np.sin(angle)

        x_obs_end_dist = (NUM_PTS_OBSTACLE * CELL_SIZE + range_mes) * np.cos(angle)
        y_obs_end_dist = (NUM_PTS_OBSTACLE * CELL_SIZE + range_mes) * np.sin(angle)

        x_obs_start = x_start + x_obs_start_dist
        y_obs_start = y_start + y_obs_start_dist

        x_obs_end = x_start + x_obs_end_dist
        y_obs_end = y_start + y_obs_end_dist

        # Convert coords to map frame
        x_start_map = int(y_start / CELL_SIZE)
        y_start_map = int(x_start / CELL_SIZE)

        x_obs_start_map = int(y_obs_start / CELL_SIZE)
        y_obs_start_map = int(x_obs_start / CELL_SIZE)

        x_obs_end_map = int(y_obs_end / CELL_SIZE)
        y_obs_end_map = int(x_obs_end / CELL_SIZE)

        # Determine free and occupied space
        rr_free, cc_free = ray_trace(x_start_map, y_start_map, x_obs_start_map, y_obs_start_map)
        rr_obs, cc_obs = ray_trace(x_obs_start_map, y_obs_start_map, x_obs_end_map, y_obs_end_map)

        valid_free_mask = (rr_free >= 0) & (rr_free < map.shape[0]) & (cc_free >= 0) & (cc_free < map.shape[1])
        valid_rr_free, valid_cc_free = rr_free[valid_free_mask], cc_free[valid_free_mask]

        valid_obs_mask = (rr_obs >= 0) & (rr_obs < map.shape[0]) & (cc_obs >= 0) & (cc_obs < map.shape[1])
        valid_rr_obs, valid_cc_obs = rr_obs[valid_obs_mask], cc_obs[valid_obs_mask]

        # Update log_odds
        log_odds[valid_rr_free, valid_cc_free] -= BETA
        log_odds[valid_rr_obs, valid_cc_obs] += ALPHA

        # Update map
        map[valid_rr_free, valid_cc_free] = (self.log_odds_to_probability(log_odds[valid_rr_free, valid_cc_free]) * 100).astype(np.int8)
        map[valid_rr_obs, valid_cc_obs] = (self.log_odds_to_probability(log_odds[valid_rr_obs, valid_cc_obs]) * 100).astype(np.int8)
