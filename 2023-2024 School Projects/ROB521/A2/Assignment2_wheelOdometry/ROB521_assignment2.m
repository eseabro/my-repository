...
for i=2:numodom
    ...
    t_o = theta_odom(i-1) + dt*omega_odom(i);
    if abs(t_o) > pi
        if t_o > 0
            theta_odom(i) = t_o - 2*pi; 
        else
            theta_odom(i) = t_o + 2*pi;
        end
    else
    ...
    y_odom(i) = y_odom(i-1) + dt*v_odom(i)*sin(theta_odom(i));
    x_odom(i) = x_odom(i-1) + dt*v_odom(i)*cos(theta_odom(i));
...
for n=1:2
    ...
    for i=1:size(t_laser,1)
        ...
        % Laser --> Vehicle 
        x_v = y_l_adj.*cos_angles-0.1;
        y_v = y_l_adj.*sin_angles;

        % Vehicle --> Inertial
        x_inert = x_v*cos(theta_i) - y_v*sin(theta_i) + x_i;
        y_inert = x_v*(sin(theta_i)) + y_v*cos(theta_i) + y_i;

        if omega_interp(i) < 0.1
            if n == 1
                scatter(x_inert, y_inert, 2, 'red')
            else
                scatter(x_inert, y_inert, 2, 'blue')
